...
| 項目 | 実測 | 実装箇所 |
|---|---|---|
| 空中の腕 | 頭上保持 → **+0.45から開き始め** → +0.85でほぼ水平 → 着水直前は水平よりやや下 | `ARM_PROFILES.bf.keys` |
| 板 | 離地から着水まで**終始2本そろって平行**。開かない・交差しない・割れない | 左右の脚を同一角度で駆動（独立ノイズなし） |
| 板 | 空中は膝ほぼ伸展、板は脚のラインの延長 | レイアウト姿勢（`shape` 既定 0） |

腕プロファイルの時刻は**離地からの絶対秒**で持っている（滞空が変わっても腕の動作時間は変わらないため）。
最後のキー（+1.45）より後は保持するので、滞空1.65秒でも着水まで左右いっぱいのまま。
...
`ARM_PROFILES.cork` は bF の実測値を暫定流用している。
